...
strip_sauce() {
    ...
    # Removes everything from Ctrl-Z (0x1A) to end of file
    perl -pe 's/\x1a.*//' "$file"
}
...
get_prefix_from_original() {
    ...
    if [ -f "$text_dir/${basename}.msg" ]; then
        ...
        # No original found, use defaults
        speed_reset
        printf '%s\n' "@CLS@"
        return
    fi
    
    # Extract leading Ctrl-A codes from the original file
    # Common codes: ^Al = clear screen, ^A0 = reset attributes, ^An = normal, ^Ac = cyan, etc.
    
    # Check if file starts with Ctrl-A l (clear screen)
    if head -c 2 "$original" | grep -q $'\x01l'; then
        # Original has clear screen, use Ctrl-A l instead of @CLS@
        printf '\001l'
        speed_reset
    else
        # Use @CLS@ as fallback
...
for file in "$ART_DIR"/*.ans; do
    ...
    if [ -f "$append_file" ]; then
        # Concatenate with prefix from original (if exists) at top
        # @POFF@ disables auto-pause so long art scrolls without interruption
        # @PAUSE@ waits for keypress
        # @PON@ re-enables auto-pause for normal operation
        echo "Merging: $filename + ${basename}.append.ans -> $TEXT_DIR/$filename"
        {
            ...
            printf '%s\n' "@PAUSE@"
            printf '%s\n' "@CLS@"
            strip_sauce "$append_file"
            printf '%s\n' "@PON@"
            get_sauce_from_original "$TEXT_DIR" "$basename"
        } > "$TEXT_DIR/$filename"
    else
        # Copy with prefix from original (if exists), strip SAUCE metadata
        # Then append SAUCE from original if it had one
...
# Second pass: Process art/random directory for rotating login screens
